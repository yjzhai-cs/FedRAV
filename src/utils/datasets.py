...
class CLISADataset(data.Dataset): 
    def __init__(self, 
                root: str,
                split: str = 'train',
                transform: Optional[Callable] = None,
        ...
        self.meta_info = None
        if split == 'train':
            with open(os.path.join(root, 'cropped_lisa', 'train_meta.json'), 'r') as file:
                self.meta_info = json.load(file)
        elif split == 'test':
            with open(os.path.join(root, 'cropped_lisa', 'valid_meta.json'), 'r') as file:
                self.meta_info = json.load(file)
        else:
            raise RuntimeError(f'{split} is undefined')
    ...
    def __getitem__(self, index):
        img_path = os.path.join(self.root, 'cropped_lisa', self.meta_info['path'][index])
...
class CLISA_truncated(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
    ...
    def __build_truncated_dataset__(self,):
        ...
        clisa_dataobj = CLISADataset(root=self.root, split=split, transform=self.transform, target_transform=self.target_transform)
        data, target = [], []
        if self.dataidxs is None:
            self.dataidxs = [i for i in range(len(clisa_dataobj))]

        for idx in self.dataidxs:
            data.append(clisa_dataobj[idx][0])
            target.append(clisa_dataobj[idx][1])
