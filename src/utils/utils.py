...
from .datasets import (MNIST_truncated, MNIST_rotated, CIFAR10_truncated, CIFAR10_rotated, CIFAR100_truncated, 
                       ...
                       Vehicle10_truncated, StanfordCars_truncated, CLISA_truncated, TLight10_truncated)
...
RESULTS_DIR = PROJECT_DIR / "save_results"

def clone_parameters_without_detach(
    src: Union[OrderedDict[str, torch.Tensor], torch.nn.Module]
) -> OrderedDict[str, torch.Tensor]:
    if isinstance(src, OrderedDict):
        return OrderedDict(
            {
                name: param.clone().requires_grad_(param.requires_grad) 
...
def clone_parameters_with_detach(
    src: Union[OrderedDict[str, torch.Tensor], torch.nn.Module]
) -> OrderedDict[str, torch.Tensor]:
    if isinstance(src, OrderedDict):
        return OrderedDict(
...
def penalty_function(x: torch.Tensor, func_type: str = "typeIII") -> torch.Tensor:
    if func_type == "typeI":
        return torch.exp(-x)
    elif func_type == "typeII":
        return torch.exp(-0.5*x)
    elif func_type == "typeIII":
...
def load_clisa_data(datadir, resize=(32,32)):
    trans = [transforms.ToTensor()]
    if resize:
        trans.insert(0, transforms.Resize(resize))
    trans = transforms.Compose(trans)

    clisa_train_ds = CLISA_truncated(root=datadir, train=True, transform=trans, download=False)
    clisa_test_ds = CLISA_truncated(root=datadir, train=False, transform=trans, download=False)

    X_train, y_train = clisa_train_ds.data, clisa_train_ds.target
    X_test, y_test = clisa_test_ds.data, clisa_test_ds.target
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0, dataidxs_test=None,
    ...
    if dataset in ('mnist', 'mnist_rotated', 'femnist', 'fmnist', 'cifar10', 'cifar10_rotated', 'cifar100',
                   ...
                   'gtsrb', 'miotcd', 'vehicle10', 'stanford_cars', 'cropped_lisa', 'tlight10'):
